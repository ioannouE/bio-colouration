...
class SimCLRDataModule(pl.LightningDataModule):
    def __init__(self, config: dict):
        ...
        # Support for multiple data directories for training
        data_config = config["data"]
        if "data_dirs" in data_config:
            self.data_dirs = data_config["data_dirs"]
        else:
            # For backward compatibility
            self.data_dirs = [data_config["data_dir"]]
    ...
    def setup(self, stage: str = "train"):
        ...
        if stage == 'fit':
            if len(self.data_dirs) > 1:
                ...
                self.dataset_train = torch.utils.data.ConcatDataset(datasets)
            else:
        ...
        if stage == "predict":
            ...
            if len(self.data_dirs) > 1:
                ...
                self.dataset_predict = torch.utils.data.ConcatDataset(datasets)   
            else:
...
class SimCLRModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log("train_loss_ssl", loss, on_step=True, on_epoch=True, sync_dist=True)
...
def generate_embeddings(model, dataloader):
    ...
    with torch.no_grad():
        with torch.cuda.amp.autocast():
            for img,_, fnames in tqdm(dataloader): # use progress bar
...
def main(args):
    ...
    # Configure trainer with multi-GPU settings
    train_config = config["train"].copy() if "train" in config else {}
    
    # Auto-detect GPUs and use all available ones
    if "devices" not in train_config:
        train_config["devices"] = "auto"  # Use all available GPUs
    
    # Set strategy for multi-GPU training if multiple GPUs are available
    if "strategy" not in train_config and torch.cuda.device_count() > 1:
        train_config["strategy"] = "ddp"  # Use DistributedDataParallel for multi-GPU
    ...
    # Adjust batch size based on number of GPUs to avoid OOM
    if num_gpus > 1:
        original_batch_size = config.get("dataloader", {}).get("batch_size", 32)
        # Reduce batch size per GPU when using multiple GPUs to avoid OOM
        adjusted_batch_size = max(4, original_batch_size // (num_gpus * 2))
        print(f"Multi-GPU training: Adjusting batch size from {original_batch_size} to {adjusted_batch_size} per GPU")
        config["dataloader"]["batch_size"] = adjusted_batch_size
    
    # Add gradient accumulation to compensate for smaller batch size
    if num_gpus > 1 and "accumulate_grad_batches" not in train_config:
        train_config["accumulate_grad_batches"] = 2
        print(f"Enabling gradient accumulation with factor {train_config['accumulate_grad_batches']}")
    
    # Enable auto find batch size if we still have memory issues
    if num_gpus > 0 and train_config.get("auto_scale_batch_size", False):
        print("Auto batch size scaling is enabled")
    
    # Set precision to 16-bit to reduce memory usage
    if num_gpus > 0 and "precision" not in train_config:
        train_config["precision"] = "16-mixed"
        print("Using 16-bit mixed precision to reduce memory usage")
    
    # Set up dataloader and model for training
    dm = SimCLRDataModule(config)
    model = SimCLRModel(config["model"])
    
    trainer = pl.Trainer(**train_config, logger=logger)
