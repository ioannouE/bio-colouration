...
from simclr.scripts.hyperspectral_data import HyperspectralBilDataset
...
def calculate_channel_stats(dataloader, num_channels):
    ...
    for images, filenames in tqdm(dataloader, desc="Calculating Stats"):
        ...
        # We want to calculate stats per channel, across all pixels and images
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate channel variance for hyperspectral data.')
    parser.add_argument('--dataset-path', type=str, required=True, help='Path to the root of the hyperspectral dataset.')
    parser.add_argument('--output-path', type=str, required=True, help='Path to the directory to save outputs.')
    parser.add_argument('--top-n', type=int, default=10, help='Number of top channels to select based on variance.')
    ...
    # --- 1. Load Dataset ---
    # No transforms needed, we want the raw data
    dataset = HyperspectralBilDataset(input_dir=args.dataset_path)
    if len(dataset) == 0:
        print(f"Error: No .bil files found in {args.dataset_path}")
        return
    ...
    # Re-create dataloader to start from the beginning
    dataloader = DataLoader(dataset, batch_size=1, shuffle=False, num_workers=args.num_workers)
    ...
    full_stats_path = os.path.join(args.output_path, 'channel_stats.csv')
    ...
    top_n_path = os.path.join(args.output_path, f'top_{args.top_n}_channels.csv')
    ...
    plt.bar(stats_df['channel'], stats_df['variance'], color='steelblue')
    plt.title('Variance per Hyperspectral Channel')
    plt.xlabel('Channel Index')
    plt.ylabel('Variance')
    plt.grid(axis='y', linestyle='--', alpha=0.7)
    plt.tight_layout()
    plot_path = os.path.join(args.output_path, 'channel_variance.png')
